...
export class Evaluator {
    ...
    VariableDeclaration(node) {
        ...
        return new CompletionRecord("normal", new JSUndefiend);
    }
    ...
    NumdericLiteral(node) {
        ...
        while(l--) {
            let c = str.charCodeAt(str.length - 1 - l);
            if(c >= 'a'.charCodeAt(0)) {
    ...
    ObjectLiteral(node) {
        ...
        if(node.children.length === 3) {
            ...
            this.PropertyList(node.chuldren[1], object);
            return object;
        }
    }
    ...
    Property(node, object) {
        ...
        } else if(node.children[0].type === "StrongLiteral") {
    ...
    MemberExpression(node) {
        ...
        if(node.children.length === 3) {
            ...
            let prop = this.evaluate(node.children[2].name);
            if("value" in prop)
                return prop.value;
            if("get" in prop)
    ...
    FunctionDeclaration(node) {
        ...
        let code = this.ecs[node.children.length - 2];
